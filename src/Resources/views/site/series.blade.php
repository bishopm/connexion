	  ...
	  <div class="col-md-6">
	  	  ...
	  	  <table class="table table-responsive table-striped">
		      @forelse ($series->sermons as $sermon)
		      	  <tr><td>{{$sermon->servicedate}}</td><td><a href="{{route('websermon',array($series->slug,$sermon->slug))}}">{{$sermon->title}}</a></td><td>{{$sermon->individual->firstname}} {{$sermon->individual->surname}}</td></tr>
		      @empty
