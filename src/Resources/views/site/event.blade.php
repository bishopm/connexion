	...
	<div class="row">
		...
		<div class="col-md-6">
			...
			@if (Auth::check())
				...
				@endforeach
			@else
				<p><a class="btn btn-primary btn-flat" href="{{url('/')}}/register">Register</a> or <button class="btn btn-primary btn-flat" data-toggle="modal" data-target="#modal-login" data-action-target="{{ route('login') }}"><i class="fa fa-login"></i>Login</button> to view event members and venue</p>
			@endif
			</ul>
		</div>
		@if (Auth::check())
